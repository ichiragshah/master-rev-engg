...
async function registerClient(data) {
  const { name, username, password_enc, telegram_username, threshold, alert_type } = data;
  const { rows } = await pool.query(
    `INSERT INTO clients (name, username, password_enc, telegram_username, threshold, alert_type)
     VALUES ($1, $2, $3, $4, $5, $6)
    ...
    [name, username, password_enc, telegram_username, threshold || 50000, alert_type || 'exposure_only']
  );
  return rows[0];
}
...
async function updateClientConfig(chat_id, field, value) {
  const allowed = ['threshold', 'alert_type', 'active'];
...
async function getAllClients() {
  const { rows } = await pool.query(
    'SELECT id, name, username, telegram_chat_id, telegram_username, threshold, alert_type, active, user_id, created_at FROM clients'
  );
  return rows;
}
